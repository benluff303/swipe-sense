...
# app.py
# FastAPI service: loads cached embeddings, serves /next and /feedback.
...
# Config: quick toggle
# ---------------------------
# True = skip heavy stages (inspect/quality/semantic gates).
# Use this while validating the pipeline with cached E/paths.
SKIP_HEAVY_STAGES = True
...
path2idx: Dict[str, int] = {}  # fast lookup (path -> idx)
...
def startup_event():
    ...
    # 1) Discover dataset (prefer cached paths.npy)
    P_CACHE = EMB_DIR / "paths.npy"
    if P_CACHE.exists():
        paths = np.load(P_CACHE, allow_pickle=True).tolist()
        print(f"[startup] Using cached paths.npy: {len(paths)}")
    else:
        if USE_GCS:
    ...
    assert len(paths) > 0, "No images found. Check emb_cache/paths.npy or GCS/local dirs."
    print(f"[startup] Found images: {len(paths)}")

    if not SKIP_HEAVY_STAGES:
        # === full pipeline ===
        ...
        fingerprint = {
            "model": MODEL_ID, "use_gcs": USE_GCS,
        ...
        E_local = l2_normalize(E_loaded, axis=1).astype(np.float32)
        assert E_local.shape[0] == len(paths)
        print(f"[startup] E shape: {E_local.shape} | loaded_from_cache={loaded}")

        print("image quality scores start")
        qf = build_or_load_qf_meta(paths)
        Q_NORM_local = quality_scores_from_qf(qf)
        ...
        if PLACES_ONLY_DEFAULT:
            ps = place_scores(E_local, pos_places, neg_places)
        ...
        # === light mode (fast): no inspect/quality/semantic gates ===
        print("[startup] SKIP_HEAVY_STAGES=True -> skipping inspect/quality/semantic gates")
        ahash_groups = {}
        fingerprint = {
            "model": MODEL_ID, "use_gcs": USE_GCS,
            "bucket": GCS_BUCKET if USE_GCS else None,
        ...
        assert E.shape[0] == len(paths), f"E={E.shape[0]} vs paths={len(paths)}"
        print(f"[startup] E shape: {E.shape} | loaded_from_cache={loaded}")

        mask = np.ones(len(paths), dtype=bool)
        Q_NORM = np.ones(len(paths), dtype=np.float32)
    ...
    rec = ImageRecommender(
        ...
        focus_gamma=FOCUS_GAMMA_DEFAULT, diversity_last_k=DIVERSITY_LAST_K_DEFAULT,
        diversity_min_cos=DIVERSITY_MIN_COS_DEFAULT, hide_exact_dupes=HIDE_EXACT_DUPES_DEFAULT
    )
    # Decay setup
